...
local T1UIColor = {
    ...
    ["Logo Image"] = "rbxassetid://116766438673467",
...
Library_Function.Gui.Name = 'Axver Hub GUI'
...
btnHide.Image = "rbxassetid://116766438673467"
...
Library.ToggleUI = function()
	...
	if game.CoreGui:FindFirstChild("Axver Hub GUI") then
		for a, b in ipairs(game.CoreGui:GetChildren()) do
			if b.Name == "Axver Hub GUI" then
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "Axver Hub"
	...
	TextLabelMain.Text = "<font color=\"rgb(0, 80, 200)\">Axver Hub</font> <font color=\"rgb(255,255,255)\">- Blox Fruit</font>"
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    return keybindFunction
        ...
        function pagefunc:AddDiscordInvite(setting)
            local prevSec = _curSec
            local sec = pageFunction:AddSection(setting.Name or "Tiktok")
            pcall(function()
                sec:AddButton({
                    Title    = "Join Tiktok",
                    Callback = function()
...
local Window = Library:CreateWindow({
    Title = "Axver Hub",
    SubTitle = "- Blox Fruit",
    SaveFolder = "AxverHub.json",
    Image = "rbxassetid://116766438673467"
})

local InfoTab = Window:AddTab("Tab | Tiktok")
...
InfoTab:AddButton({
	Title = "Follow My account tiktok",
	Callback = function()
		setclipboard("https://tiktok/axver.hub")
		Library:Notify({
			Title = "Axver Hub",
			Content = "The TikTok link has been copied",
			Duration = 4
		})
	end
})
...
InfoTab:AddParagraph({
	Title = "Community",
	Desc = "Join my account tiktok to know more, Love You All :3 !!!"
})
...
-- ============================================
-- AUTO FARM LEVEL MODULE (TỪ FILE ĐẦU)
-- ============================================

-- Hàm lấy thông tin quest theo level (từ file đầu)
function QuestNeta()
    CheckQuest()
    return {
        [1] = Mon,
...
-- Biến toàn cục
_G.EclipseAcceptQuest = _G.EclipseAcceptQuest or true
_G.EclipseLevel = false
_G.EclipseStartFarm = false
_G.FarmDistance = _G.FarmDistance or 35

-- Pos dùng để tween lên không trung khi farm
Pos = CFrame.new(0, _G.FarmDistance or 35, 0)

-- Xoay vị trí nếu bật Spin Position
local angle = 0
task.spawn(function()
    while task.wait(0.05) do
        if _G.Settings.Setting["Spin Position"] then
            ...
            local x = math.cos(radian) * radius
            local z = math.sin(radian) * radius
            Pos = CFrame.new(x, _G.FarmDistance or 35, z)
            angle = (angle + 30) % 360
        else
            Pos = CFrame.new(0, _G.FarmDistance or 35, 0)
        end
    ...
    while task.wait(0.2) do
        if _G.EclipseLevel and _G.EclipseStartFarm then
            pcall(function()
                ...
                local monName = questInfo[1]
                local questData = questInfo[2]
                local questName = questInfo[3]
                local monPos = questInfo[4]
                local nameMon = questInfo[5]
                local questPos = questInfo[6]
                
                -- Kiểm tra level để vào Submerged Island (level 2600+)
                local playerLevel = plr.Data.Level.Value
                if playerLevel >= 2600 then
                    if hrp.Position.Y > -200 then
                        if not _G._subEntering then
                            _G._subEntering = true
                            local subPos = CFrame.new(-16417.6, 74.26, 1811.3)
                            TweenPlayer(subPos)
                            task.wait(2)
                            pcall(function()
                ...
                -- Bỏ quest nếu đang làm quest khác
                ...
                if questVisible then
                    ...
                    if questTitle and nameMon and not string.find(questTitle, nameMon) then
                ...
                if not questVisible and _G.EclipseAcceptQuest then
                    TweenPlayer(questPos)
                    task.wait(0.5)
                    if (hrp.Position - questPos.Position).Magnitude <= 10 then
                        pcall(function()
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", questName, questData)
                        end)
                        task.wait(1)
                    end
                    return
                ...
                if questVisible then
                    local closestMon = nil
                    local closestDist = math.huge
                    
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == monName and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") then
                    ...
                    if closestMon then
                        -- Kéo quái lại
                        PosMon = closestMon.HumanoidRootPart.Position
                        MonFarm = closestMon.Name
                        _B = true
                        BringEnemy()
                        ...
                        -- Di chuyển đến quái
                        local targetPos = closestMon.HumanoidRootPart.CFrame * Pos
                        if (targetPos.Position - hrp.Position).Magnitude > 8 then
                            TweenPlayer(targetPos)
                        end
                        
                        -- Vô hiệu hóa quái
                        pcall(function()
                            closestMon.Humanoid.WalkSpeed = 0
                            closestMon.HumanoidRootPart.CanCollide = false
                            closestMon.HumanoidRootPart.Size = Vector3.new(1, 1, 1)
                        end)
                        
                        -- Tấn công
                        local head = closestMon:FindFirstChild("Head") or closestMon:FindFirstChild("HumanoidRootPart")
                        if head then
                            AttackModule:AttackEnemy(head, {})
                        end
                        
                        -- Fast attack nếu bật
                        if _G.Settings.Setting["Fast Attack New"] then
                            task.wait(_G.Fast_Delay or 0.15)
                            AttackModule:AttackEnemy(head, {})
                        end
                        
                    else
                        -- Không có quái thì di chuyển đến vị trí spawn
                        TweenPlayer(monPos)
                    end
                end
            end)
...
-- Function để bật/tắt farm level
function StartFarmLevel(state)
    _G.EclipseLevel = state
    _G.EclipseStartFarm = state
    if not state then
        _G.StopTween = true
        shouldTween = false
    end
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Axver Hub", Content = "Special item found! Auto Chest stopped.", Icon = "bell", Duration = 6});
				return;
			end;
			local chests = {};
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Axver Hub", Content = "Special item found!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Axver Hub", Content = "Chest Hop: limit reached, switching server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Axver Hub", Content = "Special item found! Chest Bypass stopped.", Icon = "bell", Duration = 6});
				return;
			end;
			local chests = {};
			for _, v in pairs(workspace.ChestModels:GetChildren()) do
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Axver Hub", Content = "Chest Hop: switching server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
local function ShowResetConfirm()
	pcall(function()
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Axver Hub", Content = "Settings reset! Re-execute the script.", Icon = "bell", Duration = 6});
		end);
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Axver Hub", Content = "Style not found: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Axver Yub", Content = "Purchase sent: " .. styleName, Icon = "check", Duration = 4});
end;
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Axver Hub", Content = "Arrived at: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Axver Hub", Content = "Quest complete!", Icon = "bell", Duration = 6});
						end);
